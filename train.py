...
def main():
    ...
    # 初始化 wandb
    wandb.init(project='findtime', entity='xuanzhu_07-university-of-sydney', reinit=True, config=args)
    config = wandb.config
    ...
    if args.randpos_deform_training:
        model = net(num_classes=args.num_classes, 
                    is_n_repeat=args.is_n_repeat, 
                    normalize=dataset_normalization, 
                    reNum=args.reNum).to(device)
    else:
        model = net(num_classes=args.num_classes, 
                    normalize=dataset_normalization, 
                    pos=args.pos, 
    ...
    dummy_tensor = torch.zeros((8192, 8192, 80), dtype=torch.float32).to(device)
    logger.info(f"Allocated a dummy tensor of shape {dummy_tensor.shape} to occupy GPU memory.")
    # model = net().to(device)
