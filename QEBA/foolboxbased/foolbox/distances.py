...
class MeanSquaredDistance(Distance):
    ...
    def _calculate(self):
        ...
        # n = self.reference.size()  # 获取参考张量的第一个维度（通常是 batch size 或其他尺寸）
        # print(f"here1 {n}")  # 格式化打印
        # n = self.reference.size(0)  # 获取参考张量的第一个维度（通常是 batch size 或其他尺寸）
        # print(f"here2 {n}")  # 格式化打印
        
        n = self.reference.numel()
        print(f"Total number of elements (n): {n}")  # 应该是像素总数


        f = n * (max_ - min_)**2

        self.other = self.other.to(self.reference.device)
        diff = self.other - self.reference
        diff = diff.cpu().numpy()  # 将 diff 转换为 NumPy 数组
