...
class BoundaryAttackPlusPlus(Attack):
    ...
    * ability to specify the batch size
    ...
    def attack(
        ...
        print(f"external_dtype: {self.external_dtype}")
        print(f"internal_dtype: {self.internal_dtype}")

        assert self.internal_dtype in [torch.float32, torch.float64, np.float32, np.float64]
        assert self.external_dtype in [torch.float32, torch.float64, np.float32, np.float64]
        ...
        def decision_function(x):
            ...
            for j in range(num_batchs):
                current_batch = x[self.batch_size * j:
                                  self.batch_size * (j + 1)]
                numpy_dtype = np.float32 if self.external_dtype == torch.float32 else np.float64#+
                current_batch = current_batch.astype(numpy_dtype)
        ...
        original = a.unperturbed.to(torch.float32 if self.internal_dtype == np.float32 else torch.float64)
        perturbed = a.perturbed.to(torch.float32 if self.internal_dtype == np.float32 else torch.float64)
    ...
    def project(self, unperturbed, perturbed_inputs, alphas):
        ...
        #+
        unperturbed = unperturbed.cpu().numpy()
    ...
    def log_step(self, step, distance, message='', always=False, a=None):
        ...
        if a is not None:
            ...
            fig = plt.figure()
            if a is not None and step > 0:  # 确保 step 大于 0
                import matplotlib.pyplot as plt
                fig = plt.figure()
                
                if a is not None:
                    if a.perturbed is None:
                        print("[ERROR] a.perturbed is None, skipping plotting.")
                    return
        ...
        # 绘图逻辑
                ...
                fig.savefig('BAPP_result/step%d.png' % step)  # 确保文件名有效
                plt.close(fig)
