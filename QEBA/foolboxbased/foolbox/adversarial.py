...
class Adversarial(object):
    ...
    def set_distance_dtype(self, dtype):
        ...
        print(f"Input dtype: {dtype}, type: {type(dtype)}")
        print(f"Input unperturbed dtype: {self.__unperturbed.dtype}, type: {type(self.__unperturbed.dtype)}")
    
        # 将 torch.dtype 转为 numpy.dtype
        if isinstance(dtype, torch.dtype):
            dtype = np.dtype('float32' if dtype == torch.float32 else 'float64')
        print(f"Converted dtype: {dtype}, type: {type(dtype)}")
        # 将 unperturbed 的 dtype 也转为 numpy.dtype
        if isinstance(self.__unperturbed.dtype, torch.dtype):
            unperturbed_dtype = np.dtype('float32' if self.__unperturbed.dtype == torch.float32 else 'float64')
        else:
            unperturbed_dtype = self.__unperturbed.dtype
        print(f"Converted unperturbed dtype: {unperturbed_dtype}, type: {type(unperturbed_dtype)}")
        # 确保两者可以比较
        assert dtype >= unperturbed_dtype, \
            f"Data type mismatch: dtype={dtype}, unperturbed_dtype={unperturbed_dtype}"
         # 如果 __unperturbed 是 numpy.ndarray，用 astype
         
        if isinstance(self.__unperturbed, np.ndarray):
            self.__unperturbed_for_distance = self.__unperturbed.astype(dtype, copy=False)
        # 如果 __unperturbed 是 PyTorch Tensor，用 to
        elif isinstance(self.__unperturbed, torch.Tensor):
            torch_dtype = torch.float32 if dtype == np.float32 else torch.float64
            self.__unperturbed_for_distance = self.__unperturbed.to(dtype=torch_dtype)
        else:
            raise TypeError(f"Unsupported type for self.__unperturbed: {type(self.__unperturbed)}")
    ...
    def __new_adversarial(self, x, predictions, in_bounds):
        if not isinstance(x, torch.Tensor):
            x = torch.tensor(x)  # 转换为torch.Tensor
        x = x.clone()
    ...
    def __is_adversarial(self, x, predictions, in_bounds):
        ...
        if isinstance(is_adversarial, torch.Tensor):
            is_adversarial = bool(is_adversarial.item())
