...
class DCSModel(nn.Module):
    def __init__(self, model_path, device):
        ...
        # Mock args to load the model and get normalization
        args = argparse.Namespace(
            network='WideResNet34', #WideResNet34 ResNet18
            # --- Mimicking evaluate_nonrepeated.py args ---
            none_random_training=True, # Must be False to trigger the correct model loading branch in utils.py
            rand_deform_training=False,
            randpos_deform_training=True,  # This is the user's core method
            randpos_multi_deform_training=False,
            only_adv_randpos_training=False,
            rand_path_training=False,
            # --- Args for get_loaders ---
            data_dir='~/datasets/CIFAR100/', # This path is just a placeholder
            batch_size=128,
            dataset='cifar100', #cifar10 cifar100
            worker=4,
            # --- Other necessary args ---
            is_n_repeat=False, # Must be True to load the correct block in ResNet18_1layer
...
clean_acc, robust_acc = benchmark(model,
                                  ...
                                  n_examples=1000,  # Using 1000 examples for a quicker test run
                                  dataset='cifar100',
                                  threat_model='Linf',
                                  eps=8/255,
                                  device=device,
