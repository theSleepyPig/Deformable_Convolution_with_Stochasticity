...
from utils import evaluate_standard, load_baseline, evaluate_standard_random_weights, get_loaders, load_model, evaluate_standard_random_mask
...
def get_args():
    ...
    parser.add_argument('--reNum', default=2, type=int)    
    ...
    parser.add_argument('--pretraina', default=os.path.abspath('ckpt/cifar10/ResNet18/ckpt/model_20240805133819.pth'), type=str, help='Path to load the pretrained model')
    # 20240926190609, 20240925004445
    # ckpt/cifar10/ResNet18/ckpt/model_20240928164626.pth dcs2
...
def main():
    ...
    wandb.init(project='-Test-blacktransfer', entity='xuanzhu_07-university-of-sydney', reinit=True, config=args)
    config = wandb.config   
    ...
    print(f"Pretraina model path: {args.pretraina}")
    print(f"Does pretraina model path exist? {os.path.exists(args.pretraina)}")

    assert type(args.pretraina) == str and os.path.exists(args.pretraina)
...
# normalize=dataset_normalization,
    if args.randpos_deform_training:
        model = net(num_classes=args.num_classes, is_n_repeat=args.is_n_repeat, normalize=dataset_normalization, device = device, reNum=args.reNum).to(device)
    ...
    net2 = load_baseline(args.network)
    
    modela = net2(num_classes=args.num_classes, normalize = dataset_normalization, device = device, pos = args.pos, eot = args.eot, lb = args.lb).to(device)
    print(modela)

    # load pretrained model
    pretrained_modela = torch.load(args.pretraina, map_location=device)#['state_dict']
    
    if 'state_dict' in pretrained_modela:
        partial = pretrained_modela['state_dict']
    else:# 如果没有 'state_dict'，直接使用预训练模型
        partial = pretrained_modela

    #partial['SD_'] = partial['std']
    #partial['Mu_'] = partial['mean']

    state = modela.state_dict()
    

    pretrained_dicta = {k: v for k, v in partial.items() if k in list(state.keys()) and state[k].size() == partial[k].size()}
    state.update(pretrained_dicta)
    print("Different keys:")
    for i in modela.state_dict().keys():
        if i not in pretrained_dicta.keys():
            print(i)
    ...
    atk = torchattacks.FGSM(modela, eps=8/255)
