...
def get_args():
    ...
    parser.add_argument('--vit_pretrained_path', type=str, default=None,
                    help='Path to pretrained ViT weights (only used for ViT transformer layers)')
...
def main():
    ...
    # 初始化 wandb
    wandb.init(project='findVitpretrainc10', entity='xuanzhu_07-university-of-sydney', reinit=True, config=args)
    config = wandb.config
    ...
    if args.vit_pretrained_path is not None and os.path.exists(args.vit_pretrained_path):
        print(f"Loading ViT pretrained weights from: {args.vit_pretrained_path}")
        vit_state = torch.load(args.vit_pretrained_path, map_location=device)
        if "state_dict" in vit_state:
        ...
        print(f"Loaded {len(matched)} matched parameters from ViT pretrain.")
        print(f"⚠️ Unmatched keys (not loaded due to shape or missing):")
