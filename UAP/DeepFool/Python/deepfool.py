...
def deepfool(image, net, device, num_classes=10, overshoot=0.02, max_iter=50):
    ...
    x = pert_image.unsqueeze(0).clone().detach().requires_grad_(True)  # 确保 x 是叶子节点
    fs = net.forward(x)
    print("fs shape:", fs.shape)  # 检查 fs 维度
    print("fs dtype:", fs.dtype)
    ...
    while k_i == label and loop_i < max_iter:
        ...
        pert = np.inf
        fs_scalar = fs[0, I[0]].sum()  # 确保 backward 作用在标量上
        fs_scalar.backward(retain_graph=True)
