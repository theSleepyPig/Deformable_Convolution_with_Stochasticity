...
from utils import evaluate_standard, evaluate_standard_random_weights, get_loaders, get_224loaders,get_224loaders_c10, load_model, evaluate_standard_random_mask
...
from model.earlyCon import CNNEmbedding
from model.earlyCon_DCS import CNNEmbedding_DCS
...
def get_args():
    ...
    parser.add_argument('--pretrain', default=os.path.abspath('ckpt/cifar10/earlyVit/ckpt/model_20250328171142.pth'), type=str, help='Path to load the pretrained model')
    #这里hzx/rand_defence/ckpt/cifar100/ResNet18/ckpt/model_20240911134608.pth 20240911134847， 
    # hzx/rand_defence/ckpt/cifar10/earlyVit/ckpt/model_20250328000429.pth
    # /home/yxma/hzx/hzx/rand_defence/ckpt/cifar10/earlyVit/ckpt/model_20250326203947.pth
    ...
    # /home/yxma/hzx/hzx/rand_defence/ckpt/cifar100/WideResNet34/ckpt/weight_20240930154844.pth
...
def main():
    ...
    wandb.init(project='-Test-earlyvit', entity='xuanzhu_07-university-of-sydney', reinit=True, config=args)
    config = wandb.config   
    ...
    train_loader, test_loader, dataset_normalization = get_224loaders_c10(args.data_dir, args.batch_size, dataset=args.dataset,
                                                                   worker=args.worker)
    ...
    if args.dataset.lower() == "cifar10":
        ...
        in_features = model.classifier.in_features
        model.classifier = nn.Linear(in_features, 100).to(device)

    model.to(device)
    print("Model structure constructed.")
    ...
    if 'state_dict' in pretrained_model:
        ...
        pretrained_state = pretrained_model

    model_state = model.state_dict()
    matched = {k: v for k, v in pretrained_state.items() if k in model_state and v.size() == model_state[k].size()}
    unmatched = [k for k in pretrained_state if k not in matched]

    model_state.update(matched)
    model.load_state_dict(model_state)

    print(f"Loaded {len(matched)} matched parameters.")
    if unmatched:
        print(f"⚠️ Unmatched keys:")
        for k in unmatched:
            print(f"❌ {k}")
    ...
    print('Nature:')
    evaluate_nature_acc(device, model, test_loader, args, logger, repeat=3)
