...
def main():
    ...
    wandb.init(project='-Test-pgd-eps', entity='xuanzhu_07-university-of-sydney', reinit=True, config=args)
    config = wandb.config   
...
# normalize=dataset_normalization,
    ...
    for strength in range(0, 21, 1):
        print('PGD attacking', strength / 255)
        atk = torchattacks.PGD(model, eps=strength / 255, alpha=2 / 255, steps=20, random_start=True)
        evaluate_attack(device, model, test_loader, args, atk, 'pgd', logger, repeat=10)
        # logger.info('PGD'+str(steps)+' Acc: %.4f \t', pgd_acc)
    logger.info('Testing done.')
