...
from utils import (upper_limit, lower_limit, std, clamp, get_loaders, load_model, get_loaders_mnist, get_loaders_mnist28)
#import torchvision
...
def get_args():
    ...
    parser.add_argument('--dataset', default='cifar10', choices=['cifar10', 'cifar100','tiny-imagenet','stl10','fashion-mnist'])
...
def main():
    ...
    # 初始化 wandb
    wandb.init(project='mnist', entity='xuanzhu_07-university-of-sydney', reinit=True, config=args)
    config = wandb.config
    ...
    # get data loader
    train_loader, test_loader, dataset_normalization = get_loaders_mnist28(args.data_dir, args.batch_size,
    ...
    if args.lr_schedule == 'cyclic':
        ...
        lr_steps = args.epochs
        scheduler = torch.optim.lr_scheduler.MultiStepLR(opt, milestones=[40,70], gamma=0.1)
