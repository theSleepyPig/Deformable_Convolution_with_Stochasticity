...
def get_args():
    ...
    parser.add_argument('--pretrain', default=os.path.abspath('ckpt/cifar10/ResNet18/ckpt/model_20241113231629.pth'), type=str, help='Path to load the pretrained model')
    #这里hzx/rand_defence/ckpt/cifar100/ResNet18/ckpt/model_20240911134608.pth 20240911134847， 
    # 20240926190609, 20240925004445
    # /home/yxma/hzx/hzx/rand_defence/ckpt/cifar10/ResNet18/ckpt/model_20241113231629.pth
    ...
    # /home/yxma/hzx/hzx/rand_defence/ckpt/cifar10/ResNet18/ckpt/weight_20241110032337.pth
...
def cosine_similarity(tensor1, tensor2):
    cos = F.cosine_similarity(tensor1.view(1, -1), tensor2.view(1, -1), dim=1)
    return cos.item()
...
def evaluate_attack(device, model, test_loader, args, atk, atk_name, logger, repeat=5):
    ...
    for _ in range(repeat):
        ...
        for i in pbar:
            ...
            # 生成对抗样本并计算对抗样本的梯度
            X_adv = atk(X, y)  # 生成对抗样本
            X_adv.requires_grad = True
            output_adv = model(X_adv)
            loss_adv = F.cross_entropy(output_adv, y)
            loss_adv.backward()  # 计算对抗样本的梯度
            # print(X_adv.grad)
            grad_adv = X_adv.grad.detach().clone()  # 保存对抗样本的梯度
            X_adv.grad.zero_()  # 清空梯度  
            ...
            loss.backward()  # 计算正常输入的梯度
            grad_normal = X_adv.grad.detach().clone()  # 保存正常输入的梯度
            X_adv.grad.zero_()
            
            # 3. 计算余弦相似性
            cosine_value = cosine_similarity(grad_adv, grad_normal)
            # print(cosine_value)
            cosine_similarities.append(cosine_value)
    ...
    mean_cosine = np.mean(cosine_similarities)  # 计算余弦相似性的均值
    std_cosine = np.std(cosine_similarities)    # 计算余弦相似性的标准差
    
    # 将余弦相似性值写入 CSV 文件
    with open("cosine_similarities_pg.csv", mode="w", newline="") as file:
        writer = csv.writer(file)
        writer.writerow(["Index", "Cosine Similarity"])  # 写入表头
        for idx, value in enumerate(cosine_similarities):
            writer.writerow([idx + 1, value])
    
    logger.info(f'{atk_name} Mean: {mean_acc:.4f}, Std: {std_acc:.4f}, Cosine Mean: {mean_cosine:.4f}, Cosine Std: {std_cosine:.4f}')
    
    wandb.log({f"{atk_name}_mean_adv": mean_acc, f"{atk_name}_std_adv": std_acc, f"{atk_name}_cosine_mean": mean_cosine, f"{atk_name}_cosine_std": std_cosine})
...
def main():
    ...
    wandb.init(project='-Test-cosinesimilarty', entity='xuanzhu_07-university-of-sydney', reinit=True, config=args)
    config = wandb.config   
...
# normalize=dataset_normalization,
    ...
    pgd_acc = evaluate_attack(device, model, test_loader, args, atk, 'pgd', logger, repeat=20)
