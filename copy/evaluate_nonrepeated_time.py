...
def evaluate_nature_acc(device, model, test_loader, args, logger, repeat=5):
    ...
    for _ in range(repeat):
        ...
        logger.info(f'Natural - Run { _ + 1 }: Accuracy: {nature_acc:.4f}, Time Taken: {total_time:.2f} seconds')
...
def main():
    ...
    wandb.init(project='-Test-time', entity='xuanzhu_07-university-of-sydney', reinit=True, config=args)
    config = wandb.config   
...
# normalize=dataset_normalization,
    ...
    pgd_acc = evaluate_attack(device, model, test_loader, args, atk, 'pgd', logger, repeat=5)
    ...
    # print('BAPP_custom attacking')
    # # 初始化 Foolbox 模型
    # # fmodel = fb.PyTorchModel(model, bounds=(0, 255))
    # preprocessing = (np.array([104, 116, 123]), 1)
    # # fmodel = fb.models.KerasModel(model, bounds=(0, 255), preprocessing=preprocessing)
    ...
    # atk = BAPP_custom(
    #     fmodel,
    #     criterion=fb.criteria.Misclassification()  # 不指定具体标签
    # )
    # # 执行攻击
    # evaluate_attack(device, model, test_loader, args, atk, 'bapp_custom', logger, repeat=3, is_decision_based=True)
