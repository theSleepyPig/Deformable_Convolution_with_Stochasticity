...
def get_args():
    ...
    parser.add_argument('--pretrain', default=os.path.abspath('ckpt/cifar10/WideResNet34/ckpt/model_20241107185544.pth'), type=str, help='Path to load the pretrained model')
    ...
    # /home/yxma/hzx/hzx/rand_defence/ckpt/cifar10/ResNet18/ckpt/model_20240928164626.pth
    # /home/yxma/hzx/hzx/hzx/rand_defence/ckpt/cifar10/WideResNet34/ckpt/model_20241107185544.pth
...
def main():
    ...
    wandb.init(project='-Test-eotpgd', entity='xuanzhu_07-university-of-sydney', reinit=True, config=args)
    config = wandb.config   
    ...
    for itera in range(1, 11, 1):
        # print('PGD attacking', steps)
        # atk = torchattacks.PGD(model, eps=8 / 255, alpha=2 / 255, steps=steps, random_start=True)
        # evaluate_attack(device, model, test_loader, args, atk, 'pgd', logger, repeat=10)
        
        print('EOTPGD attacking', itera)
        atk = torchattacks.EOTPGD(model, eps=8/255, alpha=2/255, steps=20, eot_iter=itera, random_start=True)
        evaluate_attack(device, model, test_loader, args, atk, 'eotpgd', logger, repeat=3)
        # logger.info('PGD'+str(steps)+' Acc: %.4f \t', pgd_acc)
