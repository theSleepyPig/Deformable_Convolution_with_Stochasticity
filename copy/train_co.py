...
def get_args():
    ...
    parser.add_argument('--network', default='ResNet18', choices=['ResNet18', 'WideResNet34','ResNet50','Vit'], type=str)
    ...
    # parser.add_argument('--device', default='0,1,2,3', type=str, help='CUDA devices (e.g., "0,1")')
...
def main():
    ...
    # 初始化 wandb
    wandb.init(project='cosinsimilarity', entity='xuanzhu_07-university-of-sydney', reinit=True, config=args)
    config = wandb.config
    ...
    model = net(num_classes=args.num_classes, is_n_repeat=args.is_n_repeat, normalize=dataset_normalization, reNum=args.reNum).to(device)
    # model = net().to(device)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i, (X, y) in enumerate(loader):
            ...
            cosine_value = cosine_similarity(grad_1, grad_2)
            cosine_values.append(cosine_value)  # 将每次的 Cosine Similarity 值存储到列表中
            print("Cosine similarity between gradients:", cosine_value)
            wandb.log({"cosine_similarity": cosine_value})
            
            # 每次检查列表长度，达到 1000 次后计算均值和方差并终止进程
            if len(cosine_values) >= 1000:
                mean_cosine = np.mean(cosine_values)
                std_cosine = np.std(cosine_values)
                logger.info(f'Cosine Similarity Mean: {mean_cosine:.4f}, Std: {std_cosine:.4f}')
                print(f'Cosine Similarity Mean: {mean_cosine:.4f}, Std: {std_cosine:.4f}')
                # 结束进程
                sys.exit()
