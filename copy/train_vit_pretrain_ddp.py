...
from utils import (upper_limit, lower_limit, std, clamp, get_224loaders, get_224loaders_c10, get_224loaders_c10_ddp, load_model)
#import torchvision
...
from model.earlyCon import EarlyConvViT, CNNEmbedding
from model.earlyCon_DCS import EarlyConvViT, CNNEmbedding_DCS
...
def get_args():
    ...
    parser.add_argument('--optimizer', default='sgd', choices=['sgd', 'adam'], type=str, help='Optimizer to use (sgd or adam)')
    parser.add_argument('--fix', action='store_true', default=False,
                        help='to fix unchange layers weight')
...
class MyViTForImageClassification(ViTForImageClassification):
    def set_rand_mask(self):
        if hasattr(self.vit.embeddings.patch_embeddings, 'set_rand_mask'):
            self.vit.embeddings.patch_embeddings.set_rand_mask()
        else:
            raise AttributeError("Current patch_embeddings module has no method 'set_rand_mask'")
...
def main():
    ...
    def setup_for_distributed(is_master):
        ...
        def print_only_master(*args, **kwargs):
            force = kwargs.pop('force', False)
            if is_master or force:
                builtin_print(*args, **kwargs)
        __builtin__.print = print_only_master

    setup_for_distributed(not use_ddp or args.local_rank == 0)
    ...
    #         print(f"CUDA device {args.device} not available.")
    ...
    #     torch.cuda.set_device(device_ids[0])  # 设置主卡

    #     device = torch.device(f"cuda:{device_ids[0]}")
    #     print("Using GPUs:", device_ids)
    #     print("Primary GPU:", torch.cuda.get_device_name(device))
    # else:
    #     print("CUDA not available. Using CPU.")
    #     device_ids = None
    #     device = torch.device("cpu")
    ...
    # 初始化 wandb
    # wandb.init(project='findVitpretrain', entity='xuanzhu_07-university-of-sydney', reinit=True, config=args)
    # config = wandb.config
    ...
    if not use_ddp or args.local_rank == 0:
        wandb.init(project='findVitpretrain', entity='xuanzhu_07-university-of-sydney', reinit=True, config=args)
        # for h in logger.handlers[:]:  # 关键：清空已有 handler
    ...
    def setup_for_wandb_logging(is_master):
        import wandb
        real_wandb_log = wandb.log

        def wandb_log_only_master(*args, **kwargs):
            if is_master:
                return real_wandb_log(*args, **kwargs)
            else:
                return None  # 非主进程不执行 log

        wandb.log = wandb_log_only_master
    setup_for_wandb_logging(not use_ddp or args.local_rank == 0)
    ...
    #                                                                worker=args.worker,
    #                                                                norm=True)
    # train_loader, train_sampler, test_loader, dataset_normalization = get_224loaders_c10_ddp(
    # args.data_dir, args.batch_size,
    # dataset=args.dataset,
    # worker=args.worker,
    # norm=True)
    if use_ddp:
        train_loader, test_loader, dataset_normalization, train_sampler = get_224loaders_c10_ddp(
            args.data_dir, args.batch_size,
            dataset=args.dataset,
            worker=args.worker,
            norm=True)
    else:
        train_loader, test_loader, dataset_normalization = get_224loaders_c10(
            args.data_dir, args.batch_size,
            dataset=args.dataset,
            worker=args.worker,
            norm=True)
    ...
    # model = ViTModel.from_pretrained( "WinKawaks/vit-tiny-patch16-224", cache_dir="model/hf_vit_tiny")
    model = MyViTForImageClassification.from_pretrained( "WinKawaks/vit-tiny-patch16-224", cache_dir="model/hf_vit_tiny")
    # logger.info(model)
    # logger.info(vit_model)
    # pretrained_transformer = vit_model.encoder
    
    # model.embeddings.patch_embeddings = CNNEmbedding(hidden_dim=192, image_size=224).to(device)
    if args.randpos_deform_training:
        model.vit.embeddings.patch_embeddings = CNNEmbedding_DCS(hidden_dim=192, image_size=224).to(device)
    else:
        model.vit.embeddings.patch_embeddings = CNNEmbedding(hidden_dim=192, image_size=224).to(device)
    ...
    if args.fix:
        for name, param in model.named_parameters():
            if not (
                name.startswith("vit.embeddings.patch_embeddings") or 
    ...
    # model.transformer = pretrained_transformer
    ...
    for epoch in range(start_epoch, args.epochs):
        if train_sampler is not None:
            # print(f"train_sampler type: {type(train_sampler)}")
        ...
        for i, (X, y) in enumerate(loader):
            ...
            if not args.none_adv_training:
                ...
                for _ in range(args.attack_iters):
                    ...
                    # output = output.logits  # ✅ 提取出 logits 张量
