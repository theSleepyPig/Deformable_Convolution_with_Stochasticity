...
from validation_mask import validate, validate_pgd, validate_aa_mask_r, validate_aa, validate_pgd_random, validate_random, validate_pgd_mask, validate_pgd_mask_r,  validate_mask
import torchvision.models as models
from model.first_resnet_imagenet_mask22 import ResNet50
from model.first_resnet_imagenet_mask2 import ResNet50_r
from tqdm import tqdm

import wandb
from tiny_in_200 import TinyImageNet


def parse_args():
    parser = argparse.ArgumentParser(description='PyTorch ImageNet Training')
    parser.add_argument('--data', default='/mnt/ssd_1/ykwang/imagenet', help='path to dataset')
    ...
    parser.add_argument('--wandb_project', default='hole_imagenet', type=str, help='WandB project name')
...
def main():
    ...
    model = ResNet50_r(num_class= config.num, pretrained=config.pretrained, rd=config.rd).cuda()
    # model = ResNet50(num_class= config.num, pretrained=config.pretrained, rd=config.rd).cuda()
    ...
    # Optimizer:
    # optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad, model.parameters()), config.TRAIN.lr,
    ...
    if config.evaluate:
        ...
        validate_mask(val_loader, model, criterion, config, logger)
        
        # validate_aa_mask_r(val_loader, model, criterion, config, logger)
        print('AA Mask attacking')
        aa_mask_acc = validate_aa_mask_r(val_loader, model, criterion, config, logger)
        logger.info(f'AA Mask Acc: {aa_mask_acc:.4f}')

        validate_pgd_mask(val_loader, model, criterion, 10, 1.0 / 255.0, config, logger)
        # return
        # validate_aa(val_loader, model, criterion, config, logger)
    ...
    for epoch in range(config.TRAIN.start_epoch, config.TRAIN.epochs):
        ...
        if config.adv_train:
            if config.rd:
                pgd_acc = validate_pgd_mask(val_loader, model, criterion, 10, 1.0 / 255.0, config, logger)
                standard_acc = validate_mask(val_loader, model, criterion, config, logger)
            else:
            ...
            if config.rd:
                pgd_acc = validate_mask(val_loader, model, criterion, config, logger)
        ...
        if is_best:
            ...
            if config.adv_train:
                best_pgd_acc_k10 = validate_pgd_mask_r(val_loader, model, criterion, 10, 1.0 / 255.0, config, logger)
                best_pgd_acc_k50 = validate_pgd_mask_r(val_loader, model, criterion, 50, 1.0 / 255.0, config, logger)
            else:
                best_pgd_acc_k10 = 0.0
        ...
        wandb.log({
            ...
            "best_pgd_acc_k10": best_pgd_acc_k10,
    ...
    for pgd_param in config.ADV.pgd_attack:
        validate_pgd_mask(val_loader, model, criterion, pgd_param[0], pgd_param[1], config, logger)
