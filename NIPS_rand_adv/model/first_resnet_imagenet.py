...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        if self.rp:
            self.rand_weight = torch.normal(0, 1, self.w_shape).to('cuda')
            Mu_ = torch.ones(self.w_shape).to('cuda')
            SD_ = torch.ones(self.w_shape).to('cuda')
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        if self.rp:
            ...
            #print(x.device, self.SD_.device, self.rand_weight.device, self.Mu_.device)
    ...
    def set_rands(self):
        ...
        self.rand_weight.data = torch.normal(0, 1, self.w_shape).to(self.SD_.device)
