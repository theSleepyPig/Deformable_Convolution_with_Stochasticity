...
class Bottleneck22(nn.Module):
    ...
    def __init__(
        ...
        super(Bottleneck22, self).__init__()
...
class ResNetmask(nn.Module):
    ...
    def __init__(
        ...
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.layer1 = self._make_layer2(block2, 64, 1, mask= mask, device=device)
        self.layer12 = self._make_layer(block, 64, 2)
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2,
                                       dilate=replace_stride_with_dilation[0])
    ...
    def _make_layer2(self, block: Type[Union[BasicBlock, Bottleneck22]], planes: int, blocks: int,
...
def _resnet(
        ...
        pretrained: bool,
        progress: bool,
        use_resnetmask: bool = False,
        num_class= 200,
        **kwargs: Any
) -> ResNet:
    # model = ResNetmask(block, layers, **kwargs)
    if use_resnetmask:
        model = ResNetmask(block, layers, **kwargs, num_classes=num_class)  # 使用 ResNetmask
    else:
        model = ResNet(block, layers, **kwargs, num_classes=num_class)  # 使用 ResNet
    # if pretrained:
    ...
    #     state_dict = torch.load(model_path)
    #     print("-------------------Pretrained Model--------------------\n", len(state_dict.keys()))
    #     print("Keys not in model state_dict")
    #     model_state_dict = {}
    #     for i in model.state_dict().keys():
    #         if not (i in state_dict.keys()):
    #             print(i)
    #         else:
    #             model_state_dict[i] = state_dict[i]
    ...
    if pretrained:
        ...
        # 检查每个预训练参数是否与模型中的参数匹配，并根据映射关系进行转换
        for key, param in state_dict.items():
            # 将预训练模型的键名转换为当前模型的键名
            mapped_key = key
            for current_key, pretrained_key in key_mapping.items():
                if key.startswith(pretrained_key):
                    mapped_key = key.replace(pretrained_key, current_key)
                    break
            
            if mapped_key in model_state_dict:
                if model_state_dict[mapped_key].shape == param.shape:
                    filtered_state_dict[mapped_key] = param
                    matched_layers += 1
                else:
                    print(f"Skipping loading parameter {key} due to shape mismatch: "
                          f"expected {model_state_dict[mapped_key].shape}, got {param.shape}")
            else:
        ...
        print(f"Successfully matched and loaded {matched_layers} layers.")

    return model


def ResNet50(num_class = 200, pretrained=False, rd=False, rp_out_channel=0):
    return _resnet('resnet50', Bottleneck, [3, 4, 6, 3], pretrained, progress=False, num_class=num_class, use_resnetmask=rd, rp_out_channel=rp_out_channel)