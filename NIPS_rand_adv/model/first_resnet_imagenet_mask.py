...
class ResNetmask(nn.Module):
    ...
    def __init__(
        ...
        # self.layer32 = self._make_layer(block2, 256, 1, stride=2,
        #                                dilate=replace_stride_with_dilation[1])
...
class ResNetmask2(nn.Module):
    ...
    def __init__(
        ...
        rp=False,
        rp_out_channel=0) -> None:
        super(ResNetmask2, self).__init__()
...
def _resnet(
    ...
    if use_resnetmask:
        model = ResNetmask2(block, layers, **kwargs, num_classes=num_class)  # 使用 ResNetmask
    else:
