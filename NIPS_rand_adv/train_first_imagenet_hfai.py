...
#from model.first_resnet_imagenet import ResNet50
...
def main():
    ...
    if os.path.exists(os.path.join(config.output_name, 'model_latest.pth')):
        ...
        logger.info("=> Automatic resume from '{}' (epoch {}, step {})"
                    .format(model_path, checkpoint['epoch']+1, checkpoint['step']+1))
    else:
        config.TRAIN.step = 0
        logger.info("!!!\nTrain from scratch\n!!!\n")
    ...
    train_dataset = ImageNet('train', transform_train)
    train_loader = train_dataset.loader(batch_size=config.DATA.batch_size, num_workers=config.DATA.workers, shuffle=True, pin_memory=True, sampler=None)
    ...
    val_dataset = ImageNet('val', transform_val)
    val_loader = val_dataset.loader(batch_size=config.DATA.batch_size, num_workers=config.DATA.workers, shuffle=False, pin_memory=True)
    ...
    for epoch in range(config.TRAIN.start_epoch, config.TRAIN.epochs):
        ...
        if config.adv_train:
            train_pgd(train_loader, model, criterion, optimizer, epoch, logger, is_first_epoch, best_pgd_acc, standard_acc, config)
        else:
            train(train_loader, model, criterion, optimizer, epoch, logger, is_first_epoch, best_pgd_acc, standard_acc, config)
...
def train(train_loader, model, criterion, optimizer, epoch, logger, is_first_epoch, best_pgd_acc, standard_acc, config):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if i % config.TRAIN.print_freq == 0:
            ...
            save_checkpoint({
                ...
                'standard_acc': standard_acc,
                'optimizer': optimizer.state_dict(),
            }, False, config.output_name)


def train_pgd(train_loader, model, criterion, optimizer, epoch, logger, is_first_epoch, best_pgd_acc, standard_acc, config):
    mean = torch.Tensor(np.array(config.TRAIN.mean)[:, np.newaxis, np.newaxis])
    mean = mean.expand(3, config.DATA.crop_size, config.DATA.crop_size).cuda()
    std = torch.Tensor(np.array(config.TRAIN.std)[:, np.newaxis, np.newaxis])
    std = std.expand(3, config.DATA.crop_size, config.DATA.crop_size).cuda()
