...
from model.first_resnet_imagenet_mask import ResNet50
...
def parse_args():
    ...
    parser.add_argument('--wandb_project', default='test', type=str, help='WandB project name')
...
# 创建一个自定义数据集类，限制每个类的图片数量
class LimitedImageFolder(Dataset):
    def __init__(self, root, transform=None, limit_per_class=2):
        self.dataset = datasets.ImageFolder(root, transform=transform)
        self.limit_per_class = limit_per_class
        self.indices = self._get_limited_indices()
    ...
    def _get_limited_indices(self):
        ...
        for idx, (_, label) in enumerate(self.dataset.imgs):
            ...
            if len(class_to_indices[label]) < self.limit_per_class:
                class_to_indices[label].append(idx)
        indices = [idx for indices_list in class_to_indices.values() for idx in indices_list]
        return indices
...
def main():
    ...
    train_dataset = LimitedImageFolder(
        ...
        limit_per_class=2  # 限制每个类的图片数量为2
    )
    ...
    val_dataset = LimitedImageFolder(
        valdir,
        transform=transforms.Compose([
