...
# from TransferAttack.transferattack.generation.ge_advgan import GE_ADVGAN
# from TransferAttack.transferattack.advanced_objective.fmaa import FMAA
# from TransferAttack.transferattack.input_transformation.decowa import DeCowA
# from TransferAttack.transferattack.gradient.fgsm import FGSM
    ...
    for _ in range(repeat):
        ...
        for i in pbar:
            ...
            if is_uap:
                ...
                # print(f"atk original shape: {atk(X, y).shape}")
            else:
                X_adv = atk.run(X, y)
            model.load_state_dict(state_dict)
...
class BPDA1:
    def __init__(self, model, epsilon, max_steps=100, learning_rate=0.5, debug=False):
        self.model = model
    ...
    def run(self, x, y, target=None):
        """
        在模型上运行 BPDA 攻击，输入是 x 和标签 y。

        :param x: 输入张量。
        :param y: 目标标签。
        :param target: 在这个示例中未实现。
        :return: 对抗样本。
        """
        adv = x.clone().requires_grad_(True)  # 确保 adv 需要计算梯度
        ...
        for i in range(self.max_steps):
            adv_purified = adv  # 使用净化后的对抗样本
            
            output = self.model(adv_purified)
            ...
            if adv.grad is not None:
                grad = adv.grad.data
                adv = adv + self.learning_rate * grad.sign()  # 使用梯度符号更新对抗样本
                adv = torch.clamp(adv, lower, upper)  # 将对抗样本限制在有效范围内
                
                print("Predicted:", torch.argmax(output))
                print("True labels:", y)
                ...
                print("错误：对抗样本没有计算出梯度。")
                break

        return adv.detach()  # 返回对抗样本并从计算图中分离

class BPDA:
    def __init__(self, model, epsilon, max_steps=100, learning_rate=0.5, num_transformations=1, alpha=2, debug=False):
        self.model = model
        self.epsilon = epsilon
        self.max_steps = max_steps
        self.learning_rate = learning_rate
    ...
    def run(self, X, y, target=None):
        """
        在模型上运行 BPDA+EOT 攻击，输入是 X 和标签 y。
        ...
        :return: 对抗样本。
        """
        adv_images = torch.zeros_like(X).to(X.device)  # 创建一个空的对抗图像容器
        
        for t in range(self.num_transformations):
            delta = torch.zeros_like(X).to(X.device)  # 初始化 delta 对抗扰动
            delta.requires_grad = True  # 确保 delta 需要计算梯度

            # lower_limit = torch.zeros_like(X).to(X.device)
            # upper_limit = torch.ones_like(X).to(X.device)
            # lower_limit = torch.clamp(X - self.epsilon, 0, 1)  # Lower limit should be X - epsilon, clipped within [0, 1]
            # upper_limit = torch.clamp(X + self.epsilon, 0, 1)  # Upper limit should be X + epsilon, clipped within [0, 1]
            ...
            for i in range(self.max_steps):
                ...
                # 更新对抗扰动
                delta.data = delta.data + self.learning_rate * torch.sign(grad)
                delta.data= torch.clamp(delta.data, -self.epsilon, self.epsilon)
                # delta.data = torch.clamp(delta, lower_limit - X, upper_limit - X)  # 限制扰动在有效范围内
                delta.grad.zero_()  # 清空梯度

                # 如果模型预测错误，则提前终止
                # print("Predicted:", output.max(1)[1])
                # print("True labels:", y)

                # if output.max(1)[1] != y:
                if not torch.eq(output.max(1)[1], y).any():
                    if self.debug:
                        print(f'Attack successful at step {i+1}')
                    break
            
            adv_images += delta  # 累加每次生成的对抗样本

        # 计算平均对抗样本（EOT）
        adv_images /= self.num_transformations

        return adv_images.detach()  # 返回对抗样本并从计算图中分离
...
def main():
    ...
    wandb.init(project='-Test-eotbpda', entity='xuanzhu_07-university-of-sydney', reinit=True, config=args)
    config = wandb.config   
...
# normalize=dataset_normalization,
    ...
    atk = BPDA(model, epsilon=8/255, max_steps=20, learning_rate=0.5, num_transformations=1, debug=False)
    evaluate_attack(device, model, test_loader, args, atk, 'BPDA', logger, repeat=3, is_uap=True)
    
    print('Running eotBPDA Attack')
    atk = BPDA(model, epsilon=8/255, max_steps=20, learning_rate=0.5, num_transformations=3, debug=False)
    evaluate_attack(device, model, test_loader, args, atk, 'eotBPDA', logger, repeat=3, is_uap=True)
    ...
    # print('RAP attacking')
    # atk = RAP(model_name='resnet18', epsilon=8/255, alpha=2/255, epoch=400, transpoint=100, epsilon_n=16/255, alpha_n=2/255, 
    #           adv_steps=8,targeted=False, random_start=False, norm='linfty', loss='crossentropy', device=device)
    # evaluate_attack(device, model, test_loader, args, atk, 'rap', logger, repeat=3)
    # print('FGSM attacking')
    # atk = FGSM(model_name='resnet18', device=device)
    # evaluate_attack(device, model, test_loader, args, atk, 'FGSM', logger, repeat=3, is_uap=True)
    
        
    # print('FMAA attacking')
    # atk = FMAA(model_name='resnet18', device=device)
    # evaluate_attack(device, model, test_loader, args, atk, 'fmaa', logger, repeat=3, is_uap=True)
    
    # print('PGN attacking')
    # atk = PGN(model_name='resnet18', device=device)
    # evaluate_attack(device, model, test_loader, args, atk, 'pgn', logger, repeat=3, is_uap=True)
    
    
    # print('DeCowA attacking')
    # atk = DeCowA(model_name='resnet18', device=device)
    # evaluate_attack(device, model, test_loader, args, atk, 'DeCowA', logger, repeat=3, is_uap=True)
