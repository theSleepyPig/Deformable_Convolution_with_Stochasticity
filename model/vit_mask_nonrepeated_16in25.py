...
class VisionTransformer_DCN(VisionTransformer):
    def __init__(
        ...
        **kwargs
        ...
        if is_conv_stem_configs:
            ...
            # 创建配置列表
            conv_stem_configs = [
                ConvStemConfig(out_channels=hidden_dim, kernel_size=DCS_kernel_size, stride=4, padding=1),
                ConvStemConfig(out_channels=hidden_dim, kernel_size=DCS_kernel_size, stride=4, padding=1)
            ]
            ...
            self.conv_proj = nn.Sequential(
                MaskedConv2d(
                    in_channels=3, out_channels=hidden_dim, kernel_size=DCS_kernel_size, padding = 1, stride=4
                ),
                nn.Conv2d(
        ...
        if isinstance(self.conv_proj[1], nn.Conv2d):
            # Init the patchify stem
        ...
        print("nonrepeatednonrepeatednonrepeated")
    ...
    def set_rand_mask(self):
        ...
        if self.last_selected1 is not None:
            available_choices = [idx for idx in range(len(top)) if idx not in self.last_selected1]
        else:
            available_choices = list(range(len(top)))
            
        selected_top = np.random.choice(available_choices, 16, replace=False)
...
# 第二层
        ...
        # if self.last_selected2 is not None:
        #     available_choices = [idx for idx in range(len(top)) if idx not in self.last_selected2]
        # else:
        #     available_choices = list(range(len(top)))
