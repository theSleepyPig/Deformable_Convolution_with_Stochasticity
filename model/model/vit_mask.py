...
class VisionTransformer_DCN(VisionTransformer):
    def __init__(
        ...
        **kwargs
        ...
        if is_conv_stem_configs:
            ...
            # 创建配置列表
            conv_stem_configs = [
                ConvStemConfig(out_channels=hidden_dim, kernel_size=DCS_kernel_size, stride=2, padding=1),
                ConvStemConfig(out_channels=hidden_dim, kernel_size=DCS_kernel_size, stride=2, padding=1),
                ConvStemConfig(out_channels=hidden_dim, kernel_size=DCS_kernel_size, stride=2, padding=1),
                ConvStemConfig(out_channels=hidden_dim, kernel_size=DCS_kernel_size, stride=2, padding=1)
            ]
            ...
            if layerNum == 1:
                self.conv_proj = nn.Sequential(
                    MaskedConv2d(
                        in_channels=3, out_channels=hidden_dim, kernel_size=DCS_kernel_size, padding=2, stride=2
                    ),
                    nn.Conv2d(
                        in_channels=hidden_dim, out_channels=hidden_dim, kernel_size=2, stride=2
                    ),
                    nn.Conv2d(
                        in_channels=hidden_dim, out_channels=hidden_dim, kernel_size=2, stride=2
                    ),
                ...
                self.conv_proj = nn.Sequential(
                    ...
                    MaskedConv2d(
                        in_channels=hidden_dim, out_channels=hidden_dim, kernel_size=DCS_kernel_size, padding = 2, stride=2
                    ),
                    MaskedConv2d(
                        in_channels=hidden_dim, out_channels=hidden_dim, kernel_size=DCS_kernel_size, padding = 2, stride=2
                    ),
        ...
        if layerNum == 1:
            ...
            if isinstance(self.conv_proj[2], MaskedConv2d):
                # Init the patchify stem
                fan_in = self.conv_proj[2].in_channels * self.conv_proj[2].kernel_size[0] * self.conv_proj[2].kernel_size[1]
                nn.init.trunc_normal_(self.conv_proj[2].weight, std=math.sqrt(1 / fan_in))
                if self.conv_proj[2].bias is not None:
                    nn.init.zeros_(self.conv_proj[2].bias)
        
            if isinstance(self.conv_proj[3], MaskedConv2d):
                # Init the patchify stem
                fan_in = self.conv_proj[3].in_channels * self.conv_proj[3].kernel_size[0] * self.conv_proj[3].kernel_size[1]
                nn.init.trunc_normal_(self.conv_proj[3].weight, std=math.sqrt(1 / fan_in))
                if self.conv_proj[3].bias is not None:
                    nn.init.zeros_(self.conv_proj[3].bias)
    ...
    def set_rand_mask(self):
        if self.layerNum == 1:
            mask = np.zeros((5, 5))
            top = [(2, 2), (0, 0), (0, 1), (1, 0), (1, 1), (2, 0), (2, 1), (3, 0), (3, 1), (4, 0), (4, 1), 
                   (3, 2), (4, 2), (0, 3), (0, 4), (1, 3), (1, 4), (0, 2), (1, 2), (3, 3), (3, 4), 
                   (4, 3), (4, 4), (2, 3), (2, 4)]

        
            available_choices = list(range(len(top)))
            selected_top = np.random.choice(available_choices, 4, replace=False)
            self.last_selected1 = selected_top  

            # selected_top = np.random.choice(len(top), 4, replace=False)
            for pos in selected_top:
                mask[top[pos]] = 1

            mask = np.tile(mask, (self.conv_proj[0].weight.shape[0], 1, 1, 1))
            self.conv_proj[0].mask = torch.tensor(mask, dtype=torch.float32).to(self.conv_proj[0].weight.device)
    ...
    # 第二层
            ...
            available_choices = list(range(len(top)))
            selected_top = np.random.choice(available_choices, 4, replace=False)
            self.last_selected2 = selected_top  

            # selected_top = np.random.choice(len(top), 4, replace=False)
            for pos in selected_top:
                mask[top[pos]] = 1

            mask = np.tile(mask, (self.conv_proj[1].weight.shape[0], 1, 1, 1))
            self.conv_proj[1].mask = torch.tensor(mask, dtype=torch.float32).to(self.conv_proj[1].weight.device)
    ...
    # 第三层
            ...
            mask = np.tile(mask, (self.conv_proj[2].weight.shape[0], 1, 1, 1))
            self.conv_proj[2].mask = torch.tensor(mask, dtype=torch.float32).to(self.conv_proj[2].weight.device)
    ...
    # 第四层
            ...
            mask = np.tile(mask, (self.conv_proj[3].weight.shape[0], 1, 1, 1))
            self.conv_proj[3].mask = torch.tensor(mask, dtype=torch.float32).to(self.conv_proj[3].weight.device)

            if self.print_flag:
                print(self.conv_proj[0].mask) 
                print(self.conv_proj[1].mask) 
                print(self.conv_proj[2].mask) 
                print(self.conv_proj[3].mask) 
                self.print_flag = False  # 设置标志为 False，以便以后不再打印
...
def vit_b_16_mask(*, weights: Optional[ViT_B_16_Weights] = ViT_B_16_Weights.IMAGENET1K_V1, progress: bool = True, layerNum=1, **kwargs: Any) -> VisionTransformer_DCN:
    """
