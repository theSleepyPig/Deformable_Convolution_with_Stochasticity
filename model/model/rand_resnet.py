...
class Rand_Weight_DConv(DeformConv2d):
    def __init__(self, inc, outc, kernel_size=3, padding=1, stride=1, bias=None, modulation=False, lb = 2048, device=None):
...
class Rand_Pos_DConv(DeformConv2d):
    ...
    def set_rands(self, batch_size, height, width):
        # self.random_offsets = torch.rand(self.lb, 2 * self.kernel_size * self.kernel_size, 1, 1) * 2 - 1
        self.random_offsets = torch.rand(batch_size, 2 * self.kernel_size * self.kernel_size, height, width).to(self.device) * 2 - 1

    def forward(self, x):
        
        batch_size, _, height, width = x.size()
        self.set_rands(batch_size, height, width) 
        offset = self.random_offsets
...
class VarBasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, device, stride=1):
        ...
        self.bn1 = BatchNorm2d(planes)
        self.conv1 = Rand_Weight_DConv(in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=False, device=device)
        self.bn2 = BatchNorm2d(planes)
        self.conv2 = Conv2d(planes, planes, kernel_size=3, stride=1, padding=1, bias=False)
...
class ResNet3(nn.Module):
    def __init__(self, block1, block2, num_blocks, device, num_classes=10, normalize=None):
...
class ResNet4(nn.Module):
    def __init__(self, block, num_blocks, device, num_classes=10, normalize=None):
        ...
        self.in_planes = 64
        self.conv1 = Rand_Pos_DConv(3, 64, kernel_size=3, stride=1, padding=1, bias=False, device=device)
        self.bn1 = BatchNorm2d(64)
        self.layer1 = self._make_layer(block, 64, num_blocks[0], stride=1)
        self.layer2 = self._make_layer(block, 128, num_blocks[1], stride=2)
        self.layer3 = self._make_layer(block, 256, num_blocks[2], stride=2)
...
def ResNet18( device=None, num_classes=10, normalize=None, pos = 0, eot = False, lb = 2048):
    return ResNet4(BasicBlock, [2,2,2,2], normalize = normalize, num_classes=num_classes, device=device)

def ResNet18_2(num_classes=10, normalize=None, device = torch.device(0), pos = 0, eot = False, lb = 2048):
    return ResNet3(VarBasicBlock2, BasicBlock, [1,1,2,2,2], normalize = normalize, num_classes=num_classes, device=device)